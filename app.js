...
function init() {
    try {
        ...
        setInterval(() => {
            ...
            if (tasksScreen && tasksScreen.classList.contains('active')) {
                renderTasks();
            }
        }, 60000);
        ...
        if (isTelegram) {
            setTimeout(() => {
                showToast('добро пожаловать в subbs!', 'success', 2000);
            }, 500);
        }
    } catch (error) {
...
// Animate stars change
function animateStarsChange(change) {
    const starsChangeEl = document.getElementById('starsChange');
    if (!starsChangeEl) return;
    
    starsChangeEl.textContent = change > 0 ? `+${change}` : `${change}`;
    starsChangeEl.style.color = change > 0 ? '#10B981' : '#EF4444';
    starsChangeEl.classList.remove('opacity-0');
    starsChangeEl.classList.add('opacity-100');
    
    setTimeout(() => {
        starsChangeEl.classList.remove('opacity-100');
        starsChangeEl.classList.add('opacity-0');
    }, 2000);
}
...
function updateUI() {
    ...
    // Animate stars change
    if (starsBalanceEl && previousStars !== state.stars) {
        const change = state.stars - previousStars;
        if (change !== 0) {
            animateStarsChange(change);
        }
        previousStars = state.stars;
    }
    
    if (starsBalanceEl) {
        starsBalanceEl.textContent = state.stars;
        // Add pulse animation if stars increased
        if (previousStars < state.stars) {
            starsBalanceEl.classList.add('pulse');
            setTimeout(() => starsBalanceEl.classList.remove('pulse'), 1000);
        }
    }
    ...
    const progress = state.completedTasks.length;
...
// Show specific screen with smooth transition
function showScreen(screenId) {
    const targetScreen = document.getElementById(screenId);
    if (!targetScreen) {
        console.error(`Screen ${screenId} not found`);
        showToast('Экран не найден', 'error');
        return;
    }
    ...
    if (isTelegram && tg && tg.BackButton) {
...
function renderTasks() {
    ...
    // Add fade-in class for smooth rendering
    tasksList.classList.add('fade-in');
    ...
    setTimeout(() => {
        tasksList.classList.remove('fade-in');
    }, 300);
...
function subscribeToChannel(channelId) {
    ...
    if (!existingSub) {
        ...
        // Show notification
        showToast('подписка зарегистрирована! проверка займёт 7 дней. не отписывайтесь!', 'success');
        ...
        showToast('подписка обновлена! проверка займёт 7 дней с начала. не отписывайтесь!', 'success');
        ...
        showToast('подписка уже зарегистрирована!', 'info');
    }
}
...
function claimGift(giftType) {
    if (state.completedTasks.length < REQUIRED_TASKS) {
        showToast('выполните все задания сначала!', 'warning');
        return;
    }
    
    if (state.receivedGifts.includes('heart')) {
        showToast('вы уже получили этот подарок!', 'info');
        return;
    }
    ...
    if (!canReceiveGifts()) {
        ...
        if (failedSubs.length > 0) {
            showToast('у вас есть отписки! подпишитесь снова и подождите неделю для проверки.', 'error');
        } else {
            showToast('дождитесь завершения проверки всех подписок (7 дней).', 'warning');
        }
        return;
    }
    ...
    if (state.completedTasks.length < REQUIRED_TASKS) {
        showToast('не все задания выполнены!', 'warning');
        return;
    }
    ...
    showToast(`поздравляем! вы получили ${GIFT_REWARD} ⭐`, 'success');
...
function renderGifts() {
    ...
    giftsList.innerHTML = '<div class="col-span-2"><div class="skeleton h-32"></div></div>';
    ...
    if (isTelegram && tg.openInvoice) {
        ...
        tg.openInvoice(invoice, (status) => {
            if (status === 'paid') {
                ...
                showToast(`поздравляем! вы получили ${gift.emoji} ${gift.name}!`, 'success');
                ...
                showToast('оплата не прошла. попробуйте снова.', 'error');
        ...
        if (state.stars < gift.cost) {
            showToast('недостаточно звёзд!', 'warning');
            return;
        }
...
function submitPromoOrder() {
    ...
    if (!channelLink) {
        showToast('введите ссылку на канал!', 'warning');
    ...
    if (!telegramLinkPattern.test(channelLink)) {
        showToast('неверная ссылка на канал! используйте формат: https://t.me/channelname', 'error');
        channelLinkInput.focus();
        return;
    }
    
    if (isNaN(subscribersCount) || subscribersCount < MIN_PROMO_SUBSCRIBERS) {
        showToast(`минимальный заказ - ${MIN_PROMO_SUBSCRIBERS} подписчиков!`, 'warning');
        subscribersCountInput.focus();
        return;
    }
    
    if (subscribersCount > 10000) {
        showToast('максимальный заказ - 10000 подписчиков!', 'warning');
        subscribersCountInput.focus();
        return;
    }
    ...
    if (isTelegram && tg.openInvoice) {
        ...
        tg.openInvoice(invoice, (status) => {
            if (status === 'paid') {
                ...
                showToast(`заказ создан! стоимость: ${totalCost} ⭐`, 'success');
        ...
        if (state.stars < totalCost) {
            showToast(`недостаточно звёзд! нужно ${totalCost} ⭐`, 'warning');
            return;
        }
    ...
    if (state.promoOrders.length === 0) {
        myOrders.innerHTML = `
            <div class="empty-state">
                ...
                <p class="text-xs opacity-50 mt-2">создайте первый заказ выше</p>
            </div>
        `;
        return;
    }
...
// Make all functions global at the end of file (after all functions are defined)
makeFunctionsGlobal();
...
function initializeApp() {
    try {
        // Make functions global (all functions should be defined by now)
        makeFunctionsGlobal();
        ...
        setTimeout(() => {
            try {
                ...
                console.error('Error initializing app:', error);
                // Show error to user
                const errorMsg = 'Ошибка загрузки приложения. Перезагрузите страницу.';
...
// Handle back button (after initialization)
setTimeout(() => {
    if (isTelegram && tg && tg.BackButton) {
...
function makeFunctionsGlobal() {
    ...
    window.renderPromoOrders = renderPromoOrders;
}
